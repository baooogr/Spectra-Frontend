...
export default function LensSelectionModal({ isOpen, onClose, product, supportedLensTypes = [], onConfirmAddToCart }) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.length > 0) {
          ...
          checkRxInRange(data[0].sphereRight, data[0].sphereLeft, data[0].pupillaryDistance);
        }
      }
  ...
  const checkRxInRange = (sphR, sphL, pd) => {
    ...
    const sphRFail = (minRx !== null && sphereR < minRx) || (maxRx !== null && sphereR > maxRx);
    const sphLFail = (minRx !== null && sphereL < minRx) || (maxRx !== null && sphereL > maxRx);
    const pdFail   = (minPd !== null && pdVal < minPd)   || (maxPd !== null && pdVal > maxPd);
  ...
  const handleSaveNewPrescription = async () => {
    ...
    } catch { setValidationErrors({ other: ["Lỗi kết nối mạng."] }); }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        ...
        style={{ maxWidth: '750px', textAlign: 'left', maxHeight: '95vh', overflowY: 'auto', borderRadius: '15px' }}
      >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (!selectedId) { setSelectedLensType(""); return; }
                ...
                const isSupported = supportedLensTypes.length === 0 || supportedLensTypes.some(
                  s => s.lensTypeId === (lens.id || lens.lensTypeId) ||
                       s.lensSpecification === (lens.lensSpecification || lens.typeName)
                );
              ...
              {lensTypes.map((lens) => {
                ...
                return (
                  <option
                    ...
                    style={!isSupported ? { color: "#9ca3af" } : { color: "#111827" }}
                  >
                    {lens.lensSpecification || lens.typeName} (+${lens.basePrice || 0})
            ...
            <div style={{ marginTop: '8px' }}>
              <button
                ...
                style={{
                  ...
                  padding: 0, fontSize: '13px', display: 'flex', alignItems: 'center',
                  gap: '6px', outline: 'none', fontWeight: '500'
                }}
              >
              {isLensInfoExpanded ? "Ẩn thông tin các loại tròng" : "Xem thông tin các loại tròng"}
                <span style={{ fontSize: '10px', transition: 'transform 0.3s ease', transform: isLensInfoExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>▼</span>
              </button>

              <div style={{ maxHeight: isLensInfoExpanded ? '400px' : '0', opacity: isLensInfoExpanded ? 1 : 0, overflow: 'hidden', transition: 'all 0.3s ease-in-out' }}>
                <div style={{ marginTop: '10px', padding: '12px', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px', color: '#475569', lineHeight: '1.6' }}>
          ...
          <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#fef2f2', borderRadius: '10px', border: '1px dashed #ef4444' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', borderBottom: '1px solid #fca5a5', paddingBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button
                  onClick={() => { setInputMode("saved"); setValidationErrors({ right: [], left: [], other: [] }); }}
                  style={{ padding: '6px 12px', fontSize: '12px', cursor: 'pointer', backgroundColor: inputMode === 'saved' ? '#111827' : '#fff', color: inputMode === 'saved' ? 'white' : '#111827', border: '1px solid #111827', borderRadius: '6px' }}
                >Toa đã lưu</button>
                <button
                  onClick={() => { setInputMode("new"); setValidationErrors({ right: [], left: [], other: [] }); }}
                  style={{ padding: '6px 12px', fontSize: '12px', cursor: 'pointer', backgroundColor: inputMode === 'new' ? '#111827' : '#fff', color: inputMode === 'new' ? 'white' : '#111827', border: '1px solid #111827', borderRadius: '6px' }}
                >+ Nhập mới</button>
            ...
            {inputMode === "saved" ? (
                ...
                {savedPrescriptions.length > 0 ? (
                  <select
                    ...
                    onChange={e => {
                      ...
                      const p = savedPrescriptions.find(x => x.prescriptionId === pid);
                      if (p) checkRxInRange(p.sphereRight, p.sphereLeft, p.pupillaryDistance);
              ...
              <div style={{ backgroundColor: 'white', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <select
                          ...
                          onChange={e => {
                            ...
                            checkRxInRange(e.target.value, prescriptionForm.sphereLeft, prescriptionForm.pupillaryDistance);
                          }}
                          style={{ width: '100%', padding: '4px' }}
                        >
                          {sphOptions.map(v => <option key={v} value={v}>{Number(v) > 0 ? `+${v}` : v}</option>)}
                        </select>
                      </div>
                      <div style={{ flex: 1 }}>
                        ...
                        <select
                          ...
                          onChange={e => {
                            ...
                            setPrescriptionForm({ ...prescriptionForm, cylinderRight: val, axisRight: newAxis });
                          }}
                          style={{ width: '100%', padding: '4px' }}
                        >
                          {cylOptions.map(v => <option key={v} value={v}>{Number(v) > 0 ? `+${v}` : v}</option>)}
                        </select>
                      </div>
                      <div style={{ flex: 1 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <select
                          ...
                          onChange={e => {
                            ...
                            checkRxInRange(prescriptionForm.sphereRight, e.target.value, prescriptionForm.pupillaryDistance);
                          }}
                          style={{ width: '100%', padding: '4px' }}
                        >
                          {sphOptions.map(v => <option key={v} value={v}>{Number(v) > 0 ? `+${v}` : v}</option>)}
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <select
                          ...
                          onChange={e => {
                            ...
                            setPrescriptionForm({ ...prescriptionForm, cylinderLeft: val, axisLeft: newAxis });
                          }}
                          style={{ width: '100%', padding: '4px' }}
                        >
                          {cylOptions.map(v => <option key={v} value={v}>{Number(v) > 0 ? `+${v}` : v}</option>)}
                ...
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: '13px', fontWeight: 'bold' }}>PD (mm):</label>
                    <select
                      value={prescriptionForm.pupillaryDistance}
                      onChange={e => {
                        setPrescriptionForm({ ...prescriptionForm, pupillaryDistance: e.target.value });
                        checkRxInRange(prescriptionForm.sphereRight, prescriptionForm.sphereLeft, e.target.value);
                      }}
        ...
        {requiresPrescription && rxOutOfRange && (
          <div style={{
            ...
            ⚠️ Kính không hỗ trợ thông số đơn thuốc này (SPH hoặc PD ngoài giới hạn của gọng kính).
          </div>
        )}
        ...
        <div className="modal-actions" style={{ display: 'flex', gap: '10px' }}>
          ...
          <button
            ...
            style={{ flex: 1, padding: '12px', backgroundColor: isAddToCartDisabled ? '#9ca3af' : '#111827' }}
          >
            🛒 Thêm Vào Giỏ Hàng
          </button>
        </div>

    ...
    try {
      ...
      if (res.ok) {
        ...
        if (err.errors) {
          ...
          setValidationErrors({ other: [err.message || "Lỗi nhập liệu. Vui lòng kiểm tra lại."] });
        }
      }
    } catch (err) {
      ...
      {activeTab === "prescription" && (
        <div style={{background: '#f9fafb', padding: '20px', borderRadius: '8px'}}>
          ...
          {!isAddingPrescription ? (
            <>
              <button onClick={() => { setIsAddingPrescription(true); setValidationErrors({right:[], left:[], other:[]}); }} style={{padding: '10px 20px', background: '#111827', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', marginBottom: '20px'}}>+ Thêm toa đo mắt mới</button>
            ...
            <form onSubmit={handleSavePrescription} style={{background: 'white', padding: '20px', borderRadius: '8px', border: '1px solid #e5e7eb'}}>
              ...
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px'}}>
                ...
                <div style={{border: '1px solid #fca5a5', padding: '15px', borderRadius: '6px', backgroundColor: '#fff5f5'}}>
                  ...
                  <label style={{display: 'block', fontSize: '13px', marginBottom: '5px', fontWeight: 'bold', color: Number(prescriptionForm.cylinderRight) === 0 ? '#aaa' : '#000'}}>Trục (AXIS) 0-180:</label>
                  <select 
                ...
                <div style={{border: '1px solid #93c5fd', padding: '15px', borderRadius: '6px', backgroundColor: '#eff6ff'}}>
                  ...
                  <label style={{display: 'block', fontSize: '13px', marginBottom: '5px', fontWeight: 'bold', color: Number(prescriptionForm.cylinderLeft) === 0 ? '#aaa' : '#000'}}>Trục (AXIS) 0-180:</label>
                  <select 
              ...
              <div style={{marginTop: '20px', padding: '15px', backgroundColor: '#f9fafb', borderRadius: '8px'}}>
                <label style={{fontWeight: 'bold', display: 'inline-block', marginBottom: '5px'}}>Khoảng cách đồng tử (PD - mm):</label>
                <select 
              ...
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px'}}>
                <div>
                  <label style={{display: 'block', fontSize: '13px', marginBottom: '5px', fontWeight: 'bold'}}>Tên Bác sĩ (Tùy chọn):</label>
                  <input type="text" value={prescriptionForm.doctorName} onChange={e => setPrescriptionForm({...prescriptionForm, doctorName: e.target.value})} style={{width: '100%', padding: '10px', borderRadius: '4px', border: '1px solid #ccc'}} placeholder="Nhập tên bác sĩ..." />
                </div>
                <div>
                  <label style={{display: 'block', fontSize: '13px', marginBottom: '5px', fontWeight: 'bold'}}>Phòng khám/Bệnh viện (Tùy chọn):</label>
                  <input type="text" value={prescriptionForm.clinicName} onChange={e => setPrescriptionForm({...prescriptionForm, clinicName: e.target.value})} style={{width: '100%', padding: '10px', borderRadius: '4px', border: '1px solid #ccc'}} placeholder="Nhập tên phòng khám..." />
                </div>
              </div>

              {/* Lỗi Khác */}
              {validationErrors.other?.length > 0 && (
                <div style={{ marginTop: '15px', padding: '10px', backgroundColor: '#fff1f2', color: '#be123c', borderRadius: '6px', fontSize: '13px', fontWeight: 'bold', textAlign: 'center' }}>
                  ⚠️ {validationErrors.other.join(" | ")}
                </div>
              )}

              <div style={{display: 'flex', gap: '10px', marginTop: '25px'}}>
                <button type="submit" style={{padding: '12px 25px', background: '#10b981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px'}}>Lưu Toa Thuốc</button>
                <button type="button" onClick={() => setIsAddingPrescription(false)} style={{padding: '12px 25px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px'}}>Hủy bỏ</button>
              </div>
            </form>
          )}
        </div>

...
function formatPaymentMethod(method) {
  ...
  return { label: 'Tiền mặt (COD)', icon: '💵', color: '#15803d', bg: '#f0fdf4' };
}
...
function formatPaymentStatus(status) {
  ...
  return map[status.toLowerCase()] || { label: status, color: '#374151', bg: '#f3f4f6' };
}
          ...
          {rxData && !isLoading && (
            <>
              ...
              <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '12px', color: '#78350f' }}>
                ...
                {rxData.expirationDate && (
                  <span>
                    ...
                    {rxData.isExpired && (
                      <span style={{ marginLeft: '6px', backgroundColor: '#fee2e2', color: '#b91c1c', padding: '1px 6px', borderRadius: '4px', fontWeight: 'bold', fontSize: '11px' }}>Đã hết hạn</span>
                    )}
                  </span>
                )}
              </div>
              <p style={{ marginTop: '8px', fontSize: '11px', color: '#a16207', fontStyle: 'italic', borderTop: '1px dashed #fde68a', paddingTop: '6px', marginBottom: 0 }}>
                Đây là dữ liệu số khách nhập khi đặt hàng. API hiện chưa hỗ trợ upload ảnh scan toa thuốc.
              </p>
            </>
          )}
        </div>
...
function PaymentInfoBlock({ paymentList }) {
  const COD = { label: 'Tiền mặt (COD)', icon: '💵', color: '#15803d', bg: '#f0fdf4' };

  if (!paymentList || paymentList.length === 0) {
    return (
      <p style={{ margin: '4px 0', fontSize: '14px' }}>
        <b>Phương thức:</b>{' '}
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', backgroundColor: COD.bg, color: COD.color, padding: '2px 10px', borderRadius: '20px', fontWeight: 'bold', fontSize: '13px' }}>
          {COD.icon} {COD.label}
        </span>
      </p>
    );
  ...
  const latest = paymentList.find(p => p.paymentStatus === 'completed') || paymentList[0];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
      ...
      {latest.paidAt && (
        <p style={{ margin: 0, fontSize: '13px', color: '#6b7280' }}>
          <b>Thanh toán lúc:</b> {new Date(latest.paidAt).toLocaleString('vi-VN')}
        </p>
      )}
...
function PreorderAmountCell({ order, headers, formatUSD }) {
  ...
  useEffect(() => {
    ...
    if (!id) return;
    let cancelled = false;
    fetch(`https://myspectra.runasp.net/api/Payments/preorder/${id}`, { headers })
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (cancelled || !data) return;
        const list = Array.isArray(data) ? data : [data];
        const paid = list.find(p => p.paymentStatus === 'completed') || list[0];
  ...
  return <span>{amount > 0 ? formatUSD(amount) : <span style={{ color: '#9ca3af' }}>—</span>}</span>;
}
...
export default function AdminOrders() {
  ...
  const handleViewDetails = async (id, isPreorder) => {
    ...
    const paymentEndpoint = isPreorder ? `Payments/preorder/${id}` : `Payments/order/${id}`;
      ...
      <>
        <div className="info-grid">
          <div className="info-card">
            ...
            <p style={{ margin: '6px 0', fontSize: '14px' }}><b>Họ Tên:</b> {displayName}</p>
            <p style={{ margin: '6px 0', fontSize: '14px' }}><b>SĐT:</b> {displayPhone}</p>
            {displayEmail && <p style={{ margin: '6px 0', fontSize: '14px' }}><b>Email:</b> {displayEmail}</p>}
            <p style={{ margin: '6px 0', fontSize: '14px' }}><b>Địa chỉ:</b> {displayAddress}</p>
            <p style={{ margin: '6px 0', fontSize: '14px' }}><b>Ghi chú:</b> {selectedOrder.note || 'Không có'}</p>
          </div>

          <div className="info-card">
            <h4>Thông tin đơn</h4>
            <p style={{ margin: '6px 0', fontSize: '14px' }}>
              <b>Ngày đặt:</b> {new Date(selectedOrder.orderDate || selectedOrder.createdAt).toLocaleString('vi-VN')}
            </p>
            <p style={{ margin: '6px 0', fontSize: '14px' }}>
              <b>Trạng thái đơn:</b> {getStatusBadge(selectedOrder.status)}
            </p>
            {selectedOrder.isPreorder && selectedOrder.expectedDate && (
              <p style={{ margin: '6px 0', fontSize: '14px', color: '#2563eb' }}>
                <b>Ngày mong muốn:</b> {new Date(selectedOrder.expectedDate).toLocaleString('vi-VN')}
              </p>
            )}
            <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #e5e7eb' }}>
              <p style={{ margin: '0 0 6px 0', fontSize: '14px', fontWeight: 'bold', color: '#374151' }}>
                💰 Thanh toán:
              </p>
        ...
        {itemsList.length === 0 ? (
          ...
          <div className="item-list">
            {itemsList.map((item, idx) => {
              ...
              const unitPrice      = item.orderPrice || item.preorderPrice || item.campaignPrice || item.unitPrice || item.price || 0;
                ...
                <div key={item.orderItemId || item.preorderItemId || idx} className="item-row">
                  <div className="item-details">
                    ...
                    <p className="item-meta">
                      SL: <b>{item.quantity || 1}</b> | Giá: <b>{formatUSD(unitPrice)}</b>
                    </p>
                    ...
                    {requiresRx && !prescriptionId && !embeddedRx && (
                      <p style={{ fontSize: '12px', color: '#dc2626', marginTop: '6px', fontStyle: 'italic' }}>
                        Tròng này yêu cầu đơn kính nhưng không tìm thấy dữ liệu đơn kính.
                      </p>
                    )}
                  </div>
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td className="col-price">
                          {isPreorder
                            ? <PreorderAmountCell order={order} headers={headers} formatUSD={formatUSD} />
                        ...
                        <td>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            ...
                            <select
                              ...
                              onChange={(e) => handleUpdateStatus(id, isPreorder, e.target.value)}
                            >
                        ...
                        <td className="col-action">
                          <button onClick={() => handleViewDetails(id, isPreorder)} className="btn-view">
                            Xem chi tiết
                          </button>
                        </td>
                      </tr>
          ...
          <div className="order-modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              <span className="total-price">
                {selectedOrder ? (() => {
                  const t = selectedOrder.totalAmount || selectedOrder.totalPrice;
                  if (t) return formatUSD(t);
                  const iList = selectedOrder.isPreorder
                    ? (selectedOrder.preorderItems || selectedOrder.items || selectedOrder.orderItems || [])
                    : (selectedOrder.orderItems    || selectedOrder.items || selectedOrder.orderDetails || []);
                  const calc = iList.reduce((s, it) => {
                    const p = it.orderPrice || it.preorderPrice || it.campaignPrice || it.unitPrice || it.price || 0;

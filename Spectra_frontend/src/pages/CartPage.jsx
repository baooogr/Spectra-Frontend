...
export default function CartPage() { 
  ...
  const formatVND = (usd) => { 
    const vnd = usd * 26250; 
    return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND', currencyDisplay: 'code' }).format(vnd); 
  };
  ...
  const goCheckout = () => { 
    ...
    if (hasPreorder && hasNormal) {
        alert("Giỏ hàng của bạn đang chứa cả Hàng có sẵn và Hàng đặt trước. Vui lòng thanh toán riêng từng loại bằng cách xóa bớt 1 loại khỏi giỏ hàng!");
        return;
    }
  ...
  return ( 
    <div className="cart" style={{ padding: "40px" }}> 
      <div className="cart__container" style={{ maxWidth: "1200px", margin: "0 auto" }}> 
                  ...
                  <div key={uniqueKey} style={{ display: "flex", gap: "20px", padding: "15px", borderBottom: "1px solid #eee", alignItems: "center" }}> 
                    ...
                    <div style={{ flex: 1 }}> 
                      <h3 style={{ margin: "0 0 8px 0" }}>
                        {item.name}
                        {item.isPreorder && <span style={{ fontSize: "11px", backgroundColor: "#2563eb", color: "white", padding: "2px 6px", borderRadius: "4px", marginLeft: "10px", verticalAlign: "middle" }}>Pre-order</span>}
                      </h3> 
                      <div style={{ fontSize: "14px", color: "#555", lineHeight: "1.6" }}> 
                        ...
                        {item.isPreorder && item.estimatedDeliveryDate && (
                           <p style={{ margin: "5px 0 0", color: "#2563eb", fontSize: "12px", fontWeight: "bold" }}>Dự kiến giao: {new Date(item.estimatedDeliveryDate).toLocaleDateString('vi-VN')}</p>
                        )}
                      </div> 
                    </div> 
            ...
            <div style={{ background: "#f9fafb", padding: "20px", borderRadius: "12px", border: "1px solid #eee", position: "sticky", top: "20px" }}> 
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px", fontSize: "20px", fontWeight: "bold" }}> 
                ...
                <div style={{ textAlign: "right" }}> 
                  ...
                  <div style={{ fontSize: "14px", color: "#666", fontWeight: "normal" }}>({formatVND(subtotal)})</div> 
                </div> 
              </div> 
              <button 
                onClick={goCheckout} 
                style={{ width: "100%", padding: "15px", background: "#111827", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", marginTop: "10px", transition: "0.2s" }}
              > 
